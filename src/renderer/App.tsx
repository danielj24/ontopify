...
function App(): JSX.Element {
  ...
  const nowPlaying = usePlaybackStore((s) =>
    s.playbackState?.item != null
      ? `${s.playbackState?.item.name} - ${s.playbackState?.item.artists[0].name}`
      : '',
  )
  const albumImg = usePlaybackStore(
    (s) => s.playbackState?.item?.album?.images?.[0]?.url,
  )
  ...
  return (
    <div className="h-full w-full bg-zinc-950 relative transition-all">
      <img
        src={albumImg}
        alt=""
        className="h-full w-full object-cover absolute top-0 blur-xl pointer-events-none"
      />
      <div
        className="absolute right-0 top-0 z-30 w-10 h-10 opacity-100"
        onClick={() => console.log('kill app')}
      >
